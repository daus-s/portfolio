  ...
  const complexChangeHandler = (e) => {
    ...
    const condition = wordValuePairs.every((pair) => {
      ...
      for (let i = 0; i < pair.gyb.length; ++i) {
        if (
          !(pair.gyb[i] === "Y" || pair.gyb[i] === "G" || pair.gyb[i] === "B")
        ) {
          return false;
        }
      }
  ...
  return (
    <div
      className={
        "l-square " +
        value.toLowerCase() +
        (ghosty ? " ghosty" + (plus ? " plus" : "") : "")
      }
      style={
        isMobile ? { width: "56px", height: "56px", margin: "5px auto" } : {}
      }
    >
      {letter?.toUpperCase()}
    </div>
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: "20px" }}>
        WordleBot helped you solve todays wordle in {winCondition(w)} turns!
      </h1>
