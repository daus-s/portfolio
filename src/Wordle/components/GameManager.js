    ...
    const complexChangeHandler = (e) => {
        ...
        const condition = wordValuePairs.every((pair) => {
            ...
            for (let i = 0; i < pair.gyb.length; ++i) {
                if (!(pair.gyb[i] === "Y" || pair.gyb[i] === "G" || pair.gyb[i] === "B")) {
                    return false;
                }
            }
            return true;
...
function Grid({ bigstr }) {
    return (
        <div className="wordle-grid">
            <Row index={1} letters={bigstr.substring(0, 5)} values={bigstr.substring(5, 10)} />
            <Row index={2} letters={bigstr.substring(10, 15)} values={bigstr.substring(15, 20)} />
            <Row index={3} letters={bigstr.substring(20, 25)} values={bigstr.substring(25, 30)} />
            <Row index={4} letters={bigstr.substring(30, 35)} values={bigstr.substring(35, 40)} />
            <Row index={5} letters={bigstr.substring(40, 45)} values={bigstr.substring(45, 50)} />
            <Row index={6} letters={bigstr.substring(50, 55)} values={bigstr.substring(55, 60)} />
        </div>
    );
}
...
function WordleHeader() {
    ...
    return (
        <>
            ...
            <div className="icons" style={isMobile ? { maxWidth: "100vw" } : {}}>
                <a href="https://www.github.com/daus-s/wordle">
                    <img src="/GithubLogo.png" alt="" title="" style={{ height: "48px", width: "48px" }} />
                </a>
                <button onClick={openModal} style={{ backgroundColor: "transparent", border: "none" }}>
                    <img
                        src="/setting.png"
                        alt=""
                        title=""
                ...
                <a href="https://raw.githubusercontent.com/daus-s/wordle/main/sgb-words.txt">
                    <img src="/document.png" alt="" title="" style={{ height: "48px", width: "48px" }} />
                </a>
            </div>
        </>
    );
...
function EndGameModal({ isOpen, setMV, w, clear }) {
    ...
    return (
        ...
        >
            <h1 style={{ fontSize: "20px" }}>WordleBot helped you solve todays wordle in {winCondition(w)} turns!</h1>
            ...
            <a href={`https://twitter.com/intent/tweet?text=www.daus-s.dev%2Fwordle%20helped%20me%20solve%20today%27s%20Wordle%20in%20${turns(w)}%20turns!`}>
