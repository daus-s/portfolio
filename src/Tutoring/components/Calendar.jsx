...
function fillSchedule(arr, obj, strMode) {
    ...
    for (const document of obj) {
        ...
        let diff =
            startDate.getUTCMonth() - today.getUTCMonth() ? length(today) : 0;
...
export default function Calendar(props) {
    ...
    const retrieveData = async () => {
        try {
            ...
            let p = await getPersonalSchedule(
    ...
    useEffect(() => {
        if (!scheduleLoaded) retrieveData();
    }, []);

    useEffect(() => {
        if (!personalLoaded) retrieveData();
    }, [props.login, cookies]);
    ...
    return (
        <div className="wide gradient">
            <div
                ...
                style={
                    isMobile
                        ? { zIndex: "3", ...mobileContainerStyling }
                        : { zIndex: "3" }
                }
            >
                <div
                    className="calendar"
                    style={isMobile ? mobileCalendarStying : {}}
                >
                    ...
                    <div
                        className="loading"
                        hidden={scheduleLoaded}
                        style={scheduleLoaded ? { pointerEvents: "none" } : {}}
                    >
                        <img
                            src="/loading-gif.gif"
                            alt="loading"
