...
function generateDateTimeTuple([day, hour], dur) {
  //analagous to x,-y (looking in -z direction)
  let today = new Date();
  let start = today;
...
function TutorReqForm(props) {
  ...
  const handleSubmit = async (event) => {
    let holder = 0;
    let name = cookies.jwt ? jwtDecode(cookies.jwt).name : ""; //leaves empty fields blank
    let email = cookies.jwt ? jwtDecode(cookies.jwt).email : "";
  ...
  return (
    <div
      id="tutorReqForm"
      style={{ zIndex: "4", width: "100%", backgroundColor: "white" }}
    >
      <Modal isOpen={submissionModalStatus} style={modalStyles}>
        ...
        <p>
          You will receive an email letting you know if your appointment is
          scheduled. Thank you!
        </p>
      </Modal>
      ...
      <form onSubmit={handleSubmit} className="form" id="tutoringrequestform">
        ...
        <label className="desc">
          <div className="dt">What would you like to focus on?</div>
          <textarea
            value={desc}
        ...
        {cookies.jwt ? (
          <button
            ...
            type="submit"
            value="Submit"
            disabled={props.time[0] === 0 && props.time[1] === -4}
